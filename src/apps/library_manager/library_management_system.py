"""
Module: Echo Library Management System
Purpose: A comprehensive OOP-based system to manage book inventory,
         tracking availability, and handling user interactions.
Architecture: Uses a 'Book' object model and a 'Library' controller class.
Standard: Seriously_Codical
"""

class Book:
    """
    Represents an individual book record.
    Encapsulates book data and state-change logic (borrowing/returning).
    """
    ...
    def __str__(self):
        # Provides a human-readable representation when the object is printed
...
class Library:
    """
    Acts as the controller to manage a collection of Book objects.
    """
    total_books = 0  # Class variable to track overall inventory size
    ...
    def find_book_by_title(self, title):
        # Case-insensitive search with whitespace stripping for robust user-input handling
        for book in self.books:
...
# --- Initial Data Load ---
# In a production environment, this would likely be loaded from a JSON or Database.
    ...
    while is_running:
        ...
        if choice == 1:
            ...
            if existing_book:
                ...
                if not isbn:
                    # Auto-generate a dummy ISBN if none provided to keep data consistent
