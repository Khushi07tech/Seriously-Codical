"""
Module: RPSLS Challenge
Purpose: A GUI implementation of Rock-Paper-Scissors-Lizard-Spock.
Logic: Uses a dictionary-based win-condition map to determine game outcomes
       efficiently without nested conditional chains.
...
class RPSLS(QWidget):
    ...
    def style(self):
        ...
        # Targeting widgets via ObjectNames for clean CSS isolation
    ...
    def connecting_buttons(self):
        buttons = [self.rock_push_button, self.paper_push_button, self.scissors_push_button,
                   self.lizard_push_button, self.spock_push_button]

        for btn in buttons:
            btn.clicked.connect(self.game_logic)
    ...
    def game_logic(self):
        ...
        # Win-condition map: key (weapon) defeats values (list of targets)
        rules = {
